...
async def analyze_food(text: str, image_b64: str = None):
    # 1. OPTION: Direct n8n connection
    n8n_url = os.getenv("N8N_WEBHOOK_URL")
    
    # Fallback to internal OpenAI if n8n is not set (Safety net)
    if not n8n_url:
        logger.warning("N8N_WEBHOOK_URL not set, falling back to local GPT-4o-mini")
        return await analyze_food_local(text, image_b64)

    try:
        payload = {}
        if text: payload["text"] = text
        if image_b64: payload["image_base64"] = image_b64
        
        async with aiohttp.ClientSession() as session:
            async with session.post(n8n_url, json=payload) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    # Expecting data to be the direct JSON from OpenAI or n8n structured response
                    # If n8n returns { "content": "{...json...}" }, we might need parsing
                    return data 
                else:
...
# Keep local version as backup/testing
async def analyze_food_local(text: str, image_b64: str = None):
    try:
        client = AsyncOpenAI(
        ...
        msgs = [{"role": "system", "content": "You are a nutritional AI. Output JSON: {food_name, calories, protein, fat, carbs, rating (1-10), rating_desc (short comment)}."}]
        ...
        logger.error(f"AI Local Error: {e}")
        return None
            ...
            "history": [
                {
                    ...
                    "rating_desc": getattr(l, 'rating_desc', None)
                } 
    ...
    if ai_result:
        async with async_session() as session:
            ...
            log = FoodLog(
                ...
                rating_desc=ai_result.get("rating_desc")
            )
            session.add(log)
            await session.commit()
